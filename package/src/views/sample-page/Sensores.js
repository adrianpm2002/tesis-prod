...
const Sensores = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const selectedZona = zonas.find(z => z.id.toString() === values.zonaId);
      const sensorData = {
        ...values,
        zona: selectedZona ? selectedZona.nombre : 'Sin zona',
        zonaId: selectedZona ? selectedZona.id : null
      };

      if (currentSensorId) {
        handleUpdateSensor({ ...sensorData, id: currentSensorId });
      } else {
        handleAddSensor({ ...sensorData, id: Date.now() });
      }
      handleCloseDialog();
    }
  });
  ...
  const handleConfirmDelete = () => {
    if (sensorToDelete) {
      handleRemoveSensor(sensorToDelete);
    }
